...
class VietLawDataPipeline:
    ...
    def open_spider(self, spider):
        ...
        # Tự động tạo thư mục nếu chưa tồn tại (giúp bạn không phải tạo bằng tay)
    ...
    def process_item(self, item, spider):
        ...
        filename = get_clean_filename(None, item.get('item_id'))
        ...
        if diagram_data:
            ...
            with open(diagram_path, 'w', encoding='utf-8') as f:
                # Lưu JSON format đẹp (indent=4) để con người dễ đọc
                json.dump(diagram_data, f, ensure_ascii=False, indent=4)

        # 4. LƯU METADATA VÀO JSONL (Dành cho Milvus/MongoDB sau này)
        # Chỉ trích xuất những thứ cần thiết để làm index
        metadata_record = {
            "id": filename,  # Thêm ID làm trường gốc để dễ query
            "doc_number": item.get('doc_number'),
            "title": item.get('metadata_api', {}).get('title', 'Unknown'),
            "issue_date": item.get('metadata_api', {}).get('issueDate', ''),
            "file_html": f"{filename}.html",
            "file_diagram": f"{filename}.json",
            "metadata_api": item.get('metadata_api')  # Lưu trọn ổ data API trả về
        }
        
        # Ghi 1 dòng JSON vào file .jsonl
        self.file.write(json.dumps(metadata_record, ensure_ascii=False) + '\n')
        self.file.flush()  # Ép hệ điều hành ghi ngay lập tức xuống đĩa (tránh mất data nếu cúp điện)
