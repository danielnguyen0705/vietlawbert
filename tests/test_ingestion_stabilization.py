"""
test_ingestion_stabilization.py - Kiểm thử tích hợp tính ổn định của Spider, Consumer và Pipeline.
Xác thực khả năng chịu lỗi, kiểm dịch quan hệ không hợp lệ và quản lý offset Kafka nguyên tử.
"""
...
# Định vị thư mục gốc vietlawbert độc lập môi trường thực thi
def find_project_root() -> Path:
    cur = Path(__file__).resolve()
    for parent in cur.parents:
        if (parent / "configs").exists() or (parent / "crawler").exists() or (parent / "main.py").exists():
            return parent
...
def load_real_chunker():
    ...
    target_path = ROOT / "preprocess" / "legal_chunker.py"
    spec = importlib.util.spec_from_file_location(module_name, target_path)
    module = importlib.util.module_from_spec(spec)
    sys.modules[module_name] = module
    spec.loader.exec_module(module)
...
class ConsumerSafetyTests(unittest.TestCase):
    ...
    def test_processing_error_is_not_swallowed_or_marked_complete(self):
        consumer = self.consumer_module.LawEventConsumer.__new__(self.consumer_module.LawEventConsumer)
        consumer._process_raw_document = Mock(side_effect=ValueError("bad record"))
        consumer.pending_messages = []
        consumer.doc_commit_batch_size = 10
        consumer.documents_processed = 0
    ...
    def test_chunk_flush_is_bounded_and_removes_only_written_rows(self):
        consumer = self.consumer_module.LawEventConsumer.__new__(self.consumer_module.LawEventConsumer)
        consumer.encoder = Mock()
        consumer.encoder.encode_texts = Mock(return_value=[[0.1] * 1024, [0.2] * 1024])
        consumer.milvus_client = Mock()
        consumer.neo_client = Mock()
        ...
        # Kiểm tra tương thích cả insert_batch và upsert
        if hasattr(consumer.milvus_client, "insert_batch") and consumer.milvus_client.insert_batch.called:
            inserted_data = consumer.milvus_client.insert_batch.call_args[0][0]
        else:
            inserted_data = consumer.milvus_client.upsert.call_args.kwargs["data"]
        self.assertEqual(2, len(inserted_data))
    ...
    def test_commit_tracks_highest_offset_for_each_partition(self):
        ...
        def message(partition, offset):
            result = Mock()
            result.topic.return_value = "law-documents-v5"
        ...
        committed_offsets = consumer.consumer.commit.call_args.kwargs["offsets"]
        self.assertEqual(
            {("law-documents-v5", 0, 6), ("law-documents-v5", 1, 8)},
            set(committed_offsets),
        )
...
class LegalChunkerHierarchyTests(unittest.TestCase):
    ...
    def test_short_preamble_is_not_merged_into_first_article(self):
        chunks = self.chunker.chunk_by_dieu("Lời mở đầu.\nĐiều 1. Phạm vi điều chỉnh", "doc-1")
    ...
    def test_oversized_atomic_unit_is_bounded_without_losing_hierarchy(self):
        ...
        self.assertTrue(all(chunk.metadata.get("total_segments") == len(chunks) for chunk in chunks))
