...
class Neo4jClient:
    ...
    def __init__(
        ...
        logger.info(f"Kết nối Neo4j Bolt tại {actual_uri}...")
        ...
        with self.driver.session() as session:
            for q in queries:
                try:
                    ...
                    logger.debug(f"Trạng thái khởi tạo constraint: {e}")
    ...
    def insert_structural_batch(self, batch_data: List[Dict[str, Any]]):
        ...
        ON CREATE SET doc.name = row.source_doc,
    ...
    def insert_semantic_relations_batch(self, items_batch: List[Dict[str, Any]]) -> int:
        """Nạp các cạnh quan hệ dẫn chiếu và hiệu lực thời gian theo từng cụm quan hệ."""
        query_batches = defaultdict(list)

        for item in items_batch:
            source_id = str(item.get("item_id") or item.get("doc_id") or "")
        ...
        with self.driver.session() as session:
            for (edge_type, direction), records in query_batches.items():
                ...
                try:
                    ...
                    logger.error(f"Lỗi nạp quan hệ {edge_type}: {exc}")

        return total_inserted

    def build_cung_van_ban_relations(self):
        """Tạo cạnh quan hệ ngang hàng giữa các Điều trong cùng một văn bản luật."""
        cypher = """
