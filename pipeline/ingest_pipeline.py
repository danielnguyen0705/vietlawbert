...
Thực thi: Đọc Shards -> AST Parsing (Fallback HTML) -> Bi-Encoder (d=256) -> Bulk load Qdrant & ES.
"""
...
class IngestPipelineWorker:
    def __init__(
        ...
        # 1. Khởi tạo bộ bóc tách cú pháp phân cấp AST
        self.parser = HybridASTParser()

        # 2. Khởi tạo kết nối Qdrant và Elasticsearch
        ...
        # 3. Nạp mô hình Bi-Encoder cho tác vụ cắt lát Matryoshka d=256
        self.device = device or ("cuda" if torch.cuda.is_available() else "cpu")
        logger.info("Đang nạp mô hình biểu diễn %s trên thiết bị [%s]...", self.model_name, self.device)
    ...
    def _extract_clean_text_fallback(self, doc_record: Dict[str, Any]) -> str:
        """
        Bảo vệ dữ liệu tương thích ngược: Bóc tách text nếu Shard từ Phase 1 cũ chỉ có html_raw.
        """
        ...
        if html_raw:
            try:
                ...
                logger.debug("Lỗi giải mã html_raw: %s", exc)

        return ""

    def process_raw_shard(self, shard_file_path: str | Path) -> int:
        """Xử lý từng tệp Shard .jsonl hoặc .jsonl.gz theo cơ chế streaming mini-batch."""
        path = Path(shard_file_path)
        if not path.exists():
            logger.error("Không tìm thấy tệp shard: %s", path)
            return 0

        logger.info("Bắt đầu xử lý Shard dữ liệu: %s...", path.name)
        ...
        with opener as f:
            for line_idx, line in enumerate(f, start=1):
                ...
                try:
                    ...
                    logger.warning("Bỏ qua dòng lỗi JSON #%d tại %s: %s", line_idx, path.name, err)
                    continue
                ...
                # 1. Phân rã cây cú pháp AST phân cấp
                try:
                    ast_chunks = self.parser.parse_document(raw_text, metadata)
                except Exception as parse_err:
                    logger.warning("Lỗi phân tích cú pháp doc %s: %s", doc_id, parse_err)
                    continue

                chunk_buffer.extend(ast_chunks)
                docs_count += 1

                # 2. Xả đệm khi đủ kích thước lô
        ...
        logger.info(
            "✓ Đã hoàn tất Shard %s: %d văn bản -> %d chunks đã nạp vào Qdrant & ES.",
            path.name,
            docs_count,
    ...
    def _flush_chunks_to_storage(self, chunks: List[Dict[str, Any]]) -> None:
        """Mã hóa vector lát cắt MRL d=256 và nạp song song vào Qdrant và Elasticsearch."""
        if not chunks:
            return

        texts = [c["text"] for c in chunks]

        # Sinh Dense Vector và chuẩn hóa L2
        with torch.no_grad():
        ...
        # 1. Nạp Dense Vector Store (Qdrant)
        self.qdrant.upsert_batch(
            records=qdrant_payloads,
            collection_name=config.QDRANT_COLLECTION_NAME,
        )

        # 2. Nạp Sparse Lexical Store (Elasticsearch)
...
def main():
    ...
    parser.add_argument("--shard-path", default=str(config.STORAGE_ROOT / "raw_shards"), help="Đường dẫn thư mục hoặc file Shard")
    parser.add_argument("--model-name", default=config.BASE_MODEL_NAME, help="Tên backbone encoder")
    parser.add_argument("--dim", type=int, default=config.QDRANT_VECTOR_DIM, help="Số chiều vector (mặc định 256)")
    parser.add_argument("--batch-size", type=int, default=config.EMBED_BATCH_SIZE, help="Kích thước batch")
    parser.add_argument("--device", default=config.EMBED_DEVICE, help="Thiết bị tính toán (cuda/cpu)")
